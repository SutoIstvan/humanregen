    ...
    <form action="" method="POST">
        ...
        <div class="row">
            ...
            <div class="form-group col-lg-12">
                <label for="bookingChair">Szolgáltatás:</label>
                <select id="bookingChair" name="chair_id" class="form-control">
                    <option value="1">Humán Regenerátor Sports</option>
                    <option value="2">InBody 970s</option>
                </select>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            timeGridWeek ' }}', // Если $calendarView не определен, использовать 'timeGridWeek'
            ...
            eventClick: function(info) {
                ...
                var chairName = info.event.extendedProps.chair_id === 2 ? 'InBody 970' : 'Humán Regenerátor Sports';
                var detailsHtml = `
                        Szolgáltatás: <strong>${chairName}</strong> <br><br>
            ...
            // Visual distinction for chair 2
            // Visual distinction and layout for chairs
            eventDidMount: function(info) {
                const chairId = info.event.extendedProps.chair_id;

                // Find the harness element (parent) to control width and position
                const harness = info.el.closest('.fc-timegrid-event-harness');
                if (harness) {
                    harness.style.width = '50%';
                    if (chairId === 2) {
                        harness.style.left = '50%';
                        harness.style.right = '0%';
                        // Remove default inset if present
                        harness.classList.remove('fc-timegrid-event-harness-inset');
                    } else {
    ...
    document.getElementById('saveBookingButton').addEventListener('click', function() {
        ...
        const chairId = document.getElementById('bookingChair').value;
    ...
    // Chair selection handler - disable 60 min for chair 2
    document.getElementById('bookingChair').addEventListener('change', function() {
        const chairId = parseInt(this.value);
        const duration60Option = document.querySelector('#bookingDuration option[value="60"]');
        const durationSelect = document.getElementById('bookingDuration');

        if (chairId === 2) {
            duration60Option.disabled = true;
            // If 60 min is currently selected, switch to 30 min
            if (durationSelect.value === '60') {
                durationSelect.value = '30';
            }
        } else {
            duration60Option.disabled = false;
        }
    });
</script>
